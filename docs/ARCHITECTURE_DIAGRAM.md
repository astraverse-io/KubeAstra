...
flowchart TB
    ...
    subgraph AGENT["Agent / Orchestration (in-process)"]
        ...
        REACT --> REG["tool_registry.py<br/>Unified tool dispatch<br/>(52 tools across surfaces)"]
    end
...
sequenceDiagram
    ...
    H-->>C: tool list (52 tools, MCP surface filter applied)
...
| `tool_registry.py` | MCP server, chat router, react_loop | Tool dispatch | Single source of truth — 52 tools, per-surface filtering |
