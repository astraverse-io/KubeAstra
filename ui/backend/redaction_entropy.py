# Vendored from mcp/services/rag/redaction.py at 2026-06-22.
# Keep in sync when the MCP-side patterns change (typically only when a new
# provider's key prefix needs coverage). Self-contained copy avoids a sys.path
# dependency on the MCP package layout in dockerized deployments.
...
def redact(text: str) -> str:
    ...
    for name, pat in _PATTERNS:
        if name in _PRESERVES_PREFIX:
            out = pat.sub(lambda m, n=name: f"{m.group(1)}<REDACTED:{n}>", out)
        else:
